...
<div class="flex h-screen bg-gray-50">
	...
	<aside class="w-64 bg-white border-r border-gray-200 flex flex-col">
		...
		<nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
			...
			<!-- Link in Bio & Website Collapsible Menu -->
			<div class="space-y-1">
				<button
					onclick={() => isLinkBioExpanded = !isLinkBioExpanded}
					class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {isLinkBioActive
						? 'bg-gradient-to-r from-indigo-50 to-blue-50 text-indigo-800'
						: 'text-gray-700 hover:bg-gray-50'}"
				>
					<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={linkBioMenu.icon} />
					</svg>
					<span class="flex-1 text-left">{linkBioMenu.name}</span>
					<svg 
						class="w-4 h-4 transition-transform {isLinkBioExpanded ? 'rotate-180' : ''}" 
						fill="none" 
						stroke="currentColor" 
						viewBox="0 0 24 24"
					>
				...
				{#if isLinkBioExpanded}
					<div class="ml-8 space-y-1">
						{#each linkBioMenu.children as child}
							<a
								href={child.href}
								class="block px-3 py-2 rounded-lg text-sm font-medium transition-colors {currentPath === child.href
